...
\begin{document}
	...
	\pgfmathsetmacro{\pangolo}{35};
	\pgfmathsetmacro{\sangolo}{{90-\pangolo}};
	...
	\draw(Q)--(QY) node[midway,below] {$\cos(\ang{90}-\alpha)$};
	\draw(P)--(PX) node[midway,right] {$\sin(\alpha)$};
	\draw(Q)--(QX)node[midway,right] {$\sin(\ang{90}-\alpha)$};
	\draw [->] (0:\sraggio/4) arc (0:\sangolo:\sraggio/4);
	\draw (\sangolo/2:\sraggio/4) node[right] {$\ang{90}-\alpha$};

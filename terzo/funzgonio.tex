...
\begin{definizionet}{Arcocotangente}{}\index{Funzione!Arcocotangente!definizione}
	La funzione inversa della cotangente è l'arcocotangente $y=\arccot x$, ed è definita in$\R$. Ha come codominio l'intervallo $0<y<\pi$ come dal grafico\nobs\vref{fig:ArcCotangenteGrafico}
\end{definizionet}
...
\begin{tabular}{LL}
	\toprule
	\arcsin\sin\alpha=\alpha	&\sin\arcsin x=x  \\
	\arccos\cos\alpha=\alpha	&\cos\arccos x=x  \\
	\arctan\tan\alpha=\alpha	&\tan\arctan x=x  \\
	\arccot\cot\alpha=\alpha	&\cot\arccot x=x  \\
	\arcsin(-x)=-\arcsin x&\\
	\arccos(-x)=\ang{180}-\arccos x&\\
	\arctan(-x)=-\arctan x&\\
	\arccot(-x)=\arccot x&\\
	\bottomrule
\end{tabular}	
	\caption{Proprietà funzioni inverse}
\end{table}

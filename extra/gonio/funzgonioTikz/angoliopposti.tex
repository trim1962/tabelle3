...
\begin{document}
	...
	\pgfmathsetmacro{\mraggio}{\raggio/3};0
	...
	\draw(Q)--(QY) node[midway,below] {$\cos(-\alpha)$};
	\draw(P)--(PX) node[midway,right=10pt] {$\sen(\alpha)$};
	\draw(Q)--(QX)node[midway,right=10pt] {$\sen(-\alpha)$};
	\draw [->] (0:\sraggio/3) arc (0:\pangolo:\sraggio/3);
	\draw (\pangolo/2:\sraggio/3) node[right] {$+\alpha$};
	\draw [->] (0:\sraggio/3) arc (0:-\pangolo:\sraggio/3);
	\draw (-\pangolo/2:\sraggio/3) node[right] {$-\alpha$};
	\draw(OO)--(P);
	\draw(OO)--(Q);
